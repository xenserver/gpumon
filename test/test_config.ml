...
let string_of_result = function
  | Error (`Parse_failure msg) ->
      Printf.sprintf "Parse_failure %s" msg
  | Error (`Unknown_version version) ->
      Printf.sprintf "Unknown_version %s" version
  | Error `Does_not_exist ->
      "Does_not_exist"
  | Ok config ->
      Printf.sprintf "Ok %s" (Gpumon_config.to_string config)
...
let test_file config_file expected_result =
  ...
  assert_equal ~msg:"Unexpected result from read_config_file"
    ~printer:string_of_result expected_result actual_result
      ...
      [
        {
          device_id= 0x1234l
        ; subsystem_device_id= Any
        ; metrics= [Other Temperature; Other PowerUsage]
        }
      ; {
          device_id= 0x5678l
        ; subsystem_device_id= Match 0x9abcl
        ; metrics= [Utilisation Compute; Utilisation MemoryIO]
        }
      ]
  }
