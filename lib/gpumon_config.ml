...
let bind_map f items =
  let rec aux acc = function
    | item :: rest ->
        f item >>= fun result -> aux (result :: acc) rest
    | [] ->
        Ok (List.rev acc)
  in
  aux [] items
...
let metric_of_string str =
  ...
  | "memoryused" ->
      Ok (Memory Used)
  | "temperature" ->
      Ok (Other Temperature)
  | "powerusage" ->
      Ok (Other PowerUsage)
  | "compute" ->
      Ok (Utilisation Compute)
  | "memoryio" ->
      Ok (Utilisation MemoryIO)
  | _ ->
      Error (`Parse_failure str)
...
let metric_of_rpc = function
  | Rpc.String str ->
      metric_of_string str
  | rpc ->
      Error (`Parse_failure (Rpc.to_string rpc))

let metrics_of_rpc = function
  | Rpc.Enum metrics_rpc ->
      bind_map metric_of_rpc metrics_rpc
  | rpc ->
      Error (`Parse_failure (Rpc.to_string rpc))
...
type config_version = V1 | V2

let version_of_dict dict =
  let version_key = "version" in
  if List.mem_assoc version_key dict then
    match List.assoc version_key dict with
    | Rpc.String "2" ->
        Ok V2
    | rpc ->
        Error (`Unknown_version (Jsonrpc.to_string rpc))
  else
    Ok V1
...
let unbox_string = function
  | Rpc.String str ->
      Ok str
  | rpc ->
      Error (`Parse_failure (Jsonrpc.to_string rpc))
...
let device_type_of_v2_format = function
  | Rpc.Dict dict ->
      ...
      (* Return the constructed device type. *)
      >>= fun metrics -> Ok {device_id; subsystem_device_id; metrics}
  | rpc ->
      Error (`Parse_failure (Jsonrpc.to_string rpc))

let device_types_of_v2_format = function
  | Rpc.Enum items ->
      bind_map device_type_of_v2_format items
  | rpc ->
      Error (`Parse_failure (Jsonrpc.to_string rpc))

let of_v2_format dict =
  lookup "device_types" dict >>= device_types_of_v2_format
  >>= fun device_types -> Ok {device_types}

let of_rpc = function
  | Rpc.Dict dict -> (
      version_of_dict dict >>= function
      | V1 ->
          of_v1_format dict
      | V2 ->
          of_v2_format dict
  ...
  | _ ->
      Error (`Parse_failure "No top-level dictionary")
...
let of_file path =
  if Sys.file_exists path then
    Unixext.string_of_file path |> of_string
  else
